...
| 项目 | 说明 | 免费额度 |
|------|------|---------|
| **Supabase 账号** | 提供数据库（记忆/画像持久化） | ✅ 免费层 |
...
| 变量名 | 值 | 说明 |
|--------|-----|------|
| `CHAT_API_KEY` | `sk-xxxxxxxx` | 你的主对话 LLM API Key（兼容 OpenAI / DeepSeek / 通义 / vLLM 等） |
| `CHAT_MODEL_NAME` | `abab6.5s-chat` | 模型名称 |

如果你用的是 **OpenAI 兼容的第三方服务**（如 DeepSeek、通义千问、自建 vLLM），还需额外配置 Base URL：

| 变量名 | 值 | 说明 |
|--------|-----|------|
| `CHAT_BASE_URL` | `https://api.deepseek.com/v1` | 第三方服务的 API 地址（注意要带 `/v1`） |

> 💡 代码内部仍兼容旧的 `OPENAI_*` 变量名，但推荐使用新的 `CHAT_*` 命名（v2.0 起统一用主对话模型）。
>
...
网关支持多个 LLM 角色按用途隔离，最小化配置只需 `CHAT_*`。

| 变量前缀 | 用途 | 对应变量（前缀 + `_API_KEY` / `_BASE_URL` / `_MODEL_NAME`） |
|----------|------|-------------------------------------------------------------|
| `OPENAI_` | 默认通用模型（已废弃，仅兼容保留） | `OPENAI_API_KEY` / `OPENAI_BASE_URL` / `OPENAI_MODEL_NAME` |
